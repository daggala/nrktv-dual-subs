...
function handleCaptionMount(nodes) {
  ...
  const lineSplits = nodes.reduce(
    (acc, node) => [
      ...acc,
      acc[acc.length - 1] + node.innerText.split(" ").length,
    ],
    [0]
  );
  ...
  const message = {
    action: "translate",
    payload: { text, source_lang: "no", target_lang: globalTL },
  };
  chrome.runtime.sendMessage(message, (response) => {
    let res = response?.response;
    if (res) {
      const translatedLineSplits = lineSplits.map((orig) =>
        Math.floor(orig * (res.split(" ").length / text.split(" ").length))
      );
      Array.from(nodes).forEach((node, lineNum) => {
        const translatedLine = res
          .split(" ")
          .slice(
            translatedLineSplits[lineNum],
            translatedLineSplits[lineNum + 1]
          )
          .join(" ");
...
const LearningMode = (function () {
  function setVideoPlayState(play = true) {
    Array.from(document.querySelectorAll("video.ludo-player")).forEach(
      (player) => {
